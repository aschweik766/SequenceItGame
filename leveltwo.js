//--global variables assigned to dragStart(source) and dragOver(dropzone) targets--event.target.id--//
...
const completedDrops = document.querySelectorAll('.draggable-drop-img');
...
//--scoring function; calulate points and losses--//
...
//--testing my drag and drop event card matches boolean--//
...
//--dragEvent Listeners for each, handlers---//
...
//dragEnd----should display steps on image cards, and should calculate the scoreBoard totals--will prompt WinModal if at winning score of 5 points; else prompt TryAgain Modal.//
...
//---functions for win/reset/tryAgain prompts to end game---//
const winGameModal = () => {
...
const loseGameModal = () => {
    if (matchingCardScore < 2 && matchingCardScore > 0) {
...
const closeLoseMod = document.getElementById('closeLoseModal');
...
const closeLoseModal = () => { loseModal.style.display = 'none';}
...
closeLoseMod.addEventListener('click', closeLoseModal);
